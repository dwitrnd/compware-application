  ...
  static post = async (req, res) => {
    try {
      ...
      const existingCategory = await gallery.findOne({
        galleryCategoryName: {
          $regex: new RegExp("^" + galleryCategoryName + "$", "i"),
        },
      });
      ...
      for (const file of files) {
        if (!file || !file.name) {
          continue; // Skip files with null or undefined name
        }
        ...
        await file.mv(`./storage/${imageName}`);
      ...
      if (error.code === 11000) {
        res.status(400).json({
          status: false,
          msg: "Duplicate image name found, please try again with different images.",
        });
      } else {
        res.status(500).json({
          status: false,
  ...
  static getOne = async (req, res) => {
    ...
    try {
      ...
      if (!isValidObjectId) {
        return res
          .status(400)
          .json({ status: false, msg: "Invalid object id" });
      }
      const result = await gallery.findById(id);
      if (!result) {
        return res
          .status(404)
          .json({ status: false, msg: "Document not found" });
      }
  ...
  static deleteImage = async (req, res) => {
    try {
      ...
      if (!existingCategory) {
        return res
          .status(404)
          .json({ status: false, msg: "Gallery not found" });
      }
